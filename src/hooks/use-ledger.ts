...
export function useLedger() {
  ...
  const [purposes, setPurposes] = useState<string[]>(() => {
    const stored =
      typeof window !== "undefined" ? localStorage.getItem("purposes") : null;
    let arr = stored
      ? JSON.parse(stored)
      : [
          "อาหาร",
    ...
    try {
      ...
      if (storedTransactions) {
        const parsedTransactions = JSON.parse(storedTransactions).map(
          (t: any) => ({
            ...
            account:
              t.account && typeof t.account === "object"
                ? {
                    id: t.account.id,
                    name: t.account.name,
      ...
      if (storedPurposes) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify(defaultPurposes),
        );
      }
    } catch (error) {
  ...
  const updateAndSaveTransactions = useCallback(
  ...
  const updateAndSavePurposes = useCallback((newPurposes: string[]) => {
    ...
    const uniquePurposes = Array.from(
      new Set(newPurposes.map((p) => p.trim())),
    );
    setPurposes(uniquePurposes);
    try {
      localStorage.setItem(
        PURPOSES_STORAGE_KEY,
        JSON.stringify(uniquePurposes),
      );
    } catch (error) {
  ...
  const editPurpose = useCallback(
    (oldPurposeName: string, updated: string) => {
      // Update purpose in list
      const newPurposes = purposes.map((p) =>
        p === oldPurposeName ? updated.trim() : p,
      );
      updateAndSavePurposes(newPurposes);
      ...
      if (oldPurposeName !== updated) {
        setTransactions((prev) =>
          prev.map((t) =>
            t.purpose === oldPurposeName ? { ...t, purpose: updated } : t,
          ),
        );
      }
  ...
  const removePurpose = useCallback(
    async (purposeToRemove: string, action?: "reclassify" | "deleteAll") => {
      ...
      if (action) {
        try {
          ...
          setTransactions(
            data.updatedTransactions.map((t: any) => ({
              ...t,
          ...
          toast({
            variant: "destructive",
            title: "เกิดข้อผิดพลาดในการอัปเดตธุรกรรม",
          });
          return;
        }
      }
  ...
  const handleSaveTransaction = useCallback(
    (
      ...
      console.log(
        "DEBUG handleSaveTransaction data:",
        data,
        "typeof data:",
        typeof data,
        "data.mode:",
      ...
      if (
        !Array.isArray(data) &&
        ((data as any).mode === "transfer" ||
          (typeof (data as any).fromAccount === "string" &&
            typeof (data as any).toAccount === "string"))
      ) {
        const tx = data as any;
        console.log(
          "DEBUG [mode=transfer|fallback] tx:",
          tx,
          "accounts:",
          accounts,
        );
        const fromAcc = accounts.find(
          (acc) => String(acc.id) === String(tx.fromAccount),
        );
        const toAcc = accounts.find(
          (acc) => String(acc.id) === String(tx.toAccount),
        );
        console.log(
          "DEBUG [mode=transfer|fallback] fromAcc:",
          fromAcc,
          "toAcc:",
          toAcc,
          "typeof acc.id:",
          typeof accounts[0]?.id,
          "typeof tx.fromAccount:",
          typeof tx.fromAccount,
          "typeof tx.toAccount:",
          typeof tx.toAccount,
        );
        if (!fromAcc || !toAcc) {
          toast({
            variant: "destructive",
            title: "ไม่พบบัญชีต้นทางหรือปลายทาง",
          });
          return;
        }
        if (!tx.amount || !tx.date) {
          toast({
            ...
            description: "กรุณากรอกจำนวนเงินและวันที่",
          });
          return;
        }
        ...
        data.forEach((tx) => {
          console.log(
            ...
            tx,
            "typeof tx.accountId:",
            typeof (tx as any).accountId,
            "accounts:",
            accounts,
          );
          const selectedAccount = accounts.find(
            (acc) => String(acc.id) === String((tx as any).accountId),
          );
      ...
      const selectedAccount = accounts.find(
        (acc) => String(acc.id) === String((data as any).accountId),
      );
      if (!selectedAccount) {
      ...
      if (editingTransaction) {
        ...
        toast({
          ...
          description: `อัปเดตรายการ "${finalData.purpose}" เรียบร้อยแล้ว`,
        });
      } else {
        // Add new transaction
  ...
  const addTemplate = useCallback(
    (data: any) => {
      ...
      const finalPurpose =
        purpose === "อื่นๆ" && data.customPurpose
          ? data.customPurpose.trim()
          : purpose;
