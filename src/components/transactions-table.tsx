...
import { TrendingUp, TrendingDown, Ban, MoreHorizontal, Pencil, Trash2, Settings2 } from 'lucide-react';
...
export function TransactionsTable({ transactions, onEdit, onDelete }: TransactionsTableProps) {
  ...
  const autoResizeColumns = () => {
    ...
    // วัดความกว้าง header
    ths.forEach((th, i) => {
      colWidths[i] = Math.max(colWidths[i], th.textContent ? measureTextWidth(th.textContent, getComputedStyle(th)) : 0);
    });
    // วัดความกว้าง cell
    ...
    trs.forEach(tr => {
      Array.from(tr.children).forEach((td, i) => {
        if (i < colWidths.length) {
          colWidths[i] = Math.max(colWidths[i], td.textContent ? measureTextWidth(td.textContent, getComputedStyle(td)) : 0);
        }
      });
    });
    // เพิ่ม padding
  ...
  return (
    <div className="w-full">
      <div className="flex justify-end mb-2 gap-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-8 w-8" title="ตั้งค่าคอลัมน์">
              <Settings2 className="w-5 h-5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <div className="px-2 py-1 text-xs font-semibold text-muted-foreground">เลือกคอลัมน์ที่ต้องการแสดง</div>
            {ALL_COLUMNS.map(col => (
      ...
      <Table className="w-full table-fixed bg-transparent [&_thead]:bg-transparent [&_tbody]:bg-transparent [&_tr]:bg-transparent [&_th]:bg-transparent [&_td]:bg-transparent" ref={tableRef}>
