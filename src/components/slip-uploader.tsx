...
interface SlipUploaderProps {
  onExtractionComplete: (
    data: ExtractTransactionDetailsOutput & { validationResult?: string },
  ) => void;
}
...
export function SlipUploader({ onExtractionComplete }: SlipUploaderProps) {
  ...
  const handleFileChange = async (
    ...
    reader.onload = async () => {
      ...
      try {
        const extractedDetails = await extractTransactionDetails({
          slipDataUri: dataUri,
        });
        ...
        toast({
          ...
          description:
            "ไม่สามารถอ่านรายละเอียดสลิปได้ รูปภาพอาจไม่ชัดเจน กรุณาลองอีกครั้งหรือป้อนข้อมูลด้วยตนเอง",
        });
      } finally {
        setIsLoading(false);
        // Reset file input
      ...
      {isLoading ? (
        <>
          ...
          <p className="text-muted-foreground">
            กำลังอ่านสลิปของคุณ กรุณารอสักครู่...
          </p>
        </>
      ) : (
        <label
