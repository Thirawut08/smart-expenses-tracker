...
export function InvestmentBreakdownChart({ data }: InvestmentBreakdownChartProps) {
    const pieData = useMemo(() => {
        return data.map(item => ({ name: item.name, value: item.investment }));
    }, [data]);
    ...
    return (
        <div className="grid grid-cols-1 gap-8">
            <Card>
                <CardHeader>
                    <CardTitle>มูลค่าเงินลงทุน เทียบมูลค่าปัจจุบัน</CardTitle>
                </CardHeader>
                <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            ...
                            <Legend />
                            <Bar dataKey="investment" fill="#FFB74D" name="จำนวนเงินลงทุน" />
                            <Bar dataKey="current" fill="#81C784" name="มูลค่าปัจจุบัน" />
                        </BarChart>
                    </ResponsiveContainer>
                </CardContent>
            </Card>
            <Card>
                <CardHeader>
                    <CardTitle>มูลค่าการลงทุนรวม</CardTitle>
                </CardHeader>
                <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                        <PieChart>
