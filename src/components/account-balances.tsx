...
const BalanceTable = ({
  ...
  balances: { name: string; balance: number; currency: "THB" | "USD" }[];
  totalInThb: number;
  ...
  return (
    <div>
      <div className="text-right mb-4">
        ...
        <p
          className={`text-xl font-bold ${totalInThb >= 0 ? "text-green-600" : "text-red-600"}`}
        >
          ...
          <TableBody>
            {balances.map((acc) => (
              <TableRow key={acc.name}>
                ...
                <TableCell
                  className={`text-right font-semibold ${acc.balance >= 0 ? "" : "text-red-600"}`}
                >
...
export function AccountBalances({
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter((acc) =>
      Array.isArray(acc.types)
        ? acc.types.includes(filterType)
  ...
  const balancesByType = useMemo(() => {
    ...
    Object.entries(accountsByType).forEach(([type, accs]) => {
      ...
      const totalInThb = sortedBalances.reduce(
        (sum, acc) =>
          sum + convertToTHB(acc.balance, acc.currency, usdToThbRate || 0),
        0,
      );
      ...
      <div>
        <div className="text-right mb-4">
          <p className="text-sm text-muted-foreground">
            ยอดรวม (เทียบเท่า THB)
          </p>
          <p
            className={`text-xl font-bold ${totalAll >= 0 ? "text-green-600" : "text-red-600"}`}
          >
            {thbFormatter.format(totalAll)}
          </p>
        </div>
  ...
  const groupTypes =
    filterType && filterType !== "all"
      ? [filterType]
      : Object.keys(balancesByType);
  return (
    <Card>
      <CardHeader>
        <CardTitle>ยอดคงเหลือแต่ละบัญชี</CardTitle>
        <CardDescription>ยอดเงินคงเหลือแยกตามประเภทบัญชี</CardDescription>
      </CardHeader>
      <CardContent>
        {groupTypes.map((type) =>
          balancesByType[type] ? (
            <div key={type} className="mb-8">
              ...
              <BalanceTable
                balances={balancesByType[type].balances}
                totalInThb={balancesByType[type].totalInThb}
                noDataMessage={`ไม่พบบัญชีประเภท ${type}`}
              />
            </div>
          ) : null,
        )}
